...
需要提取的字段（每个时间块）：
1. **activity** (活动名称) - 必需，从文本中提取用户在做什么，**保持简洁**（如"吃晚饭"、"学习"、"开会"）
...
5. **description** (详细描述) - **重要**：必须包含文本中的详细信息，如：
   - 和谁一起（如"和学长一起吃"、"和同事一起"）
   - 具体内容（如"讨论项目"、"复习功课"）
   - 其他重要细节（如"线上会议"、"在咖啡厅"）
   - 如果文本中有这些信息，必须在 description 中保留，不要遗漏
   - 可以包含原始文本的关键信息，但可以适当精简
6. **tag** (标签分类) - 必需，根据活动内容自动分类为以下标签之一：**{tag_list}**

**字段提取原则**：
- **activity**：保持简洁，只提取核心活动（如"吃晚饭"、"学习"、"开会"），不要包含细节信息
- **description**：必须包含文本中的详细信息，如"和学长一起吃晚饭"、"和同事讨论项目"、"在咖啡厅学习"等
- **location**：如果文本明确提到地点，提取到 location 字段；如果地点信息在描述中（如"在咖啡厅学习"），也可以保留在 description 中
...
1. **只提取完整的时间段（重要！）**：
   ...
   - **重要**：即使文本使用"待会儿"、"一会儿"等模糊表达，只要文本中明确提到了具体时间点（如"六点到六点半"），就必须提取这个时间段，不要因为"待会儿"而返回空数组
...
3. **相对时间处理（重要！）**：
   ...
   **过去时间**：
   ...
   - 重要：这些相对时间都是过去的时间，结束时间必须是当前时间（{current_time_str}），不是未来时间
   ...
   **未来时间（重要！必须提取）**：
   - "待会儿"、"一会儿"、"等会儿" = 表示未来的时间，需要根据文本中的具体时间点提取
   - "待会儿六点到六点半" = 开始时间：18:00，结束时间：18:30（使用当前日期）
   - "一会儿八点我要开会" = 如果当前时间早于20:00，开始时间：20:00，结束时间：根据上下文推断（如20:30或21:00）
   - **重要**：即使文本使用"待会儿"、"一会儿"等模糊表达，只要文本中明确提到了具体时间点（如"六点"、"六点半"），就必须提取这个时间段
   - **示例**："待会儿六点到六点半我要和学长一块吃晚饭"
     - ✅ 正确：提取时间段 18:00-18:30，activity="吃晚饭"
     - ❌ 错误：因为"待会儿"而返回空数组 []
   - **规则**：如果文本同时包含"待会儿/一会儿"和具体时间点，优先提取具体时间点，忽略"待会儿"的模糊性
...
6. **活动名称（activity）的准确性**：
   ...
   - **保持简洁**：只提取核心活动名称，不要包含细节信息
   - ✅ 正确示例："吃晚饭"、"跑步+通勤"、"学习"、"通勤/去咖啡厅"
   - ❌ 错误示例："约了人"、"起床"、"到达"、"开始"、"和学长一起吃晚饭"（细节应放在 description 中）
   - 如果时间段包含多个活动（如"跑步+通勤"），可以用"+"或"/"连接
   - **重要**：细节信息（如"和学长一起"、"讨论项目"）应该放在 description 字段中，而不是 activity 中
...
2. **第二步：推断时间点之间的事件**
   ...
   - **活动名称（activity）应该是时间段内的活动描述，而不是时间点的动作描述**
     - **保持简洁**：只提取核心活动名称，细节信息放在 description 中
     - ✅ 正确："吃晚饭"（时间段的活动，简洁）
     - ❌ 错误："约了人"（时间点的动作）
     - ❌ 错误："和学长一起吃晚饭"（包含细节，应该简洁为"吃晚饭"，"和学长一起"放在 description 中）
     - ✅ 正确："跑步+通勤"（时间段的活动组合）
     - ❌ 错误："起床"（时间点的动作）
     - **description 必须包含详细信息**：如"和学长一起吃晚饭"、"和同事讨论项目"、"在咖啡厅学习"等
   - **避免重复提取**：
...
- 示例6：文本"待会儿六点到六点半我要和学长一块吃晚饭"
  - 时间点：18点，18点30分
  - **重要**：虽然文本使用"待会儿"（未来时间），但明确提到了具体时间点"六点到六点半"，必须提取
  - 时间段（18点-18点30分）：吃晚饭
  - **activity**：保持简洁，只提取核心活动"吃晚饭"
  - **description**：必须包含详细信息"和学长一块吃晚饭"（不要遗漏"和学长"这个信息）
  - 结果：[{{"activity": "吃晚饭", "start_time": "{current_date}T18:00:00", "end_time": "{current_date}T18:30:00", "location": null, "description": "和学长一块吃晚饭", "tag": "生活"}}]
  - ❌ 错误1：因为"待会儿"而返回空数组 []
  - ❌ 错误2：activity 写成"和学长一块吃晚饭"（应该简洁为"吃晚饭"，细节放在 description 中）
  - ❌ 错误3：description 为空或只写"吃晚饭"（应该包含"和学长"这个重要信息）
...
4. **相对时间计算**：
   - **过去时间**（如"刚刚"、"半小时前"）：结束时间必须是当前时间（{current_time_iso}），不是未来时间
   - **未来时间**（如"待会儿"、"一会儿"）：如果文本中明确提到了具体时间点（如"六点到六点半"），必须提取这个时间段，不要因为"待会儿"而返回空数组
5. **未来时间处理**：即使文本使用"待会儿"、"一会儿"等模糊表达，只要文本中明确提到了具体时间点，就必须提取这个时间段

**返回格式**：
- 如果有完整时间段：返回 JSON 数组，格式：[{{"activity": "...", "start_time": "...", "end_time": "...", "location": "...", "description": "...", "tag": "..."}}]
- 如果没有完整时间段：返回空数组：[]

**字段填写要求**：
- **activity**：简洁的核心活动名称（如"吃晚饭"、"学习"、"开会"）
- **description**：必须包含文本中的详细信息，如"和学长一起吃晚饭"、"和同事讨论项目"、"在咖啡厅学习"等，不要遗漏重要细节
- **location**：如果文本明确提到地点，提取到 location 字段
